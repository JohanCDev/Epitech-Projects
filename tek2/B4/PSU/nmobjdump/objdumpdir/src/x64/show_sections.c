...
bool print_a_sec_64(char const *tabinfo, void *data, Elf64_Shdr *shdr, int i)
{
    ...
    while (oct < shdr[i].sh_size) {
        ...
        printf("%02x", *value);
        printed_enter = show_ascii_values_64(&oct, data, shdr, i);
    }
    if (oct % 16) {
        print_spaces(oct);
...
bool is_valid_obj64(Elf64_Ehdr *elf, char const *strtab, Elf64_Shdr *shdr,
int i)
{
...
bool is_valid_ex64(Elf64_Ehdr *elf, char const *strtab, Elf64_Shdr *shdr,
int i)
{
...
bool is_valid_so64(Elf64_Ehdr *elf, char const *strtab, Elf64_Shdr *shdr,
int i)
{
...
void show_sections_64(t_objd *metadata, char const *strtab)
{
    for (int i = 0; i <= metadata->elf->e_shnum; i++) {
        if (metadata->shdr[i].sh_size &&
        (is_valid_obj64(metadata->elf, strtab, metadata->shdr, i) ||
        is_valid_ex64(metadata->elf, strtab, metadata->shdr, i) ||
        is_valid_so64(metadata->elf, strtab, metadata->shdr, i))) {
            print_a_sec_64(strtab, metadata->content, metadata->shdr, i)
            == true ? 0 : printf("\n");
        }
    }
}
